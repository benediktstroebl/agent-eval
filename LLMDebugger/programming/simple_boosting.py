...
def async_main(
    ...
    if n_proc == 1:
        for i, item in enumerate_resume(dataset, log_path, testfile=testfile):
            ...
            while step < 3:
                if step == 0:
                    model = model_factory("codellama")
                    print("Using CodeLlama at first...")
                elif step == 1:
                    model = model_factory("gpt-3.5-turbo-0613")
                    print("Switched to GPT-3.5-turbo-0613...")
                elif step == 2:
                    model = model_factory("gpt-4-1106-preview")
                    print("Switched to GPT-4-1106-preview...")
